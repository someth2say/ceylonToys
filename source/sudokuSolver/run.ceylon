...
shared class Sudoku(Integer size, Integer dimension = 2) {
	...
	{[Cell+]*} allVectorsForCoordIdx(Integer cordIdx) {
		return allCells.group((Cell cell) {
			Integer[][2] slices = cell.coords.slice(cordIdx);
			value vectorId = slices[0].append([null]).append(slices[1].rest);
			print("VectorId for ``cordIdx`` coordinate : ``vectorId``");
			return vectorId;
		}).items;
	}
	...
	shared {{[Cell+]*}+} vectors => (0..dimension-1).map(allVectorsForCoordIdx);
